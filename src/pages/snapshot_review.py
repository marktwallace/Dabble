...
def _generate(selected):
    ...
    path = Path(REPORTS_DIR) / f"snapshot_{timestamp}_{slug}.py"
    ...
    if len(selected) == 1:
        ...
        if typ == "chart":
            ...
            content = _chart_file(df, data.get("code", ""), key)
        else:
            content = _table_file(data, key)
    else:
        content = _multi_file(selected)
...
def _chart_file(df, plotly_code, key):
    title = _title(key)

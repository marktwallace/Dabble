...
A diagnostic command for inspecting and troubleshooting knowledge base retrieval. Does not inject context into Claude — it shows the developer or analyst what the KB *would* retrieve for a given query.

Two forms:
- `/kb` — uses the last user message as the query
- `/kb some query text` — probes against that specific text

Returns a table in the conversation showing: total chunk count, similarity threshold, and for each of the top matches — similarity score, whether it would pass the threshold, source file, and description. Useful for understanding why a chunk did or did not retrieve, and for verifying KB state after `/learn` or a rebuild.

**`/kb rebuild`** — clears ChromaDB and reloads all chunks from the `knowledge/` directory. Admin escape hatch for resyncing after a crash or manual file edit. The `knowledge/` directory is always the source of truth; this command brings ChromaDB back into sync with it.

...
4. Confirmed chunks are each saved as a standalone `.txt` file in `knowledge/`
...
This episodic form is more useful for retrieval than isolated facts. When Claude calls `recall_knowledge` at the start of an analysis, it can pull prior sequences that succeeded in similar situations and adapt the plan rather than assembling one from scratch. Sequences convey *how to reason* in the domain, not just *what is true*.

**The description line is the routing signal:**

Each chunk file begins with a `description:` line. At session start, Dabble scans `knowledge/` and injects a `<available_knowledge>` registry block into the system prompt — one line per chunk, listing slug and description. Claude reads this registry and decides which chunks to retrieve via `recall_knowledge` before beginning analysis. The description line must be written to fire on the right queries and not on unrelated ones. The `/learn` extraction prompt asks Claude to write it as a phrase completing "recall this when the user asks about ___."
...
The `knowledge/` directory is the durable source of truth. Each chunk is one file; the slug (filename without extension) is the address Claude uses in `recall_knowledge` calls.
...
Reloads the analytic database connection and reports the new data timestamp. In S3/DuckLake mode, re-downloads the catalog from S3 before reconnecting, picking up any data written since the session started. In local mode, reconnects to the local catalog.

Useful after an ETL run completes mid-session. The analyst types `/refresh` and gets a confirmation: `Database refreshed. Data as of: 2026-03-26 21:00 UTC`.
...
Plain `.txt` files in `knowledge/`. No vector database, no embeddings, no ML infrastructure.

Each file is one chunk. Filename is a human-readable slug derived from the description (e.g. `qc-status-codes.txt`). File format:
...
**At session start:** `knowledge_base.py` scans `knowledge/*.txt`, reads the `description:` line from each file, and builds an `<available_knowledge>` registry block injected into the system prompt. If `knowledge/` is empty or absent, the block is omitted.

**Retrieval:** Claude reads the registry, decides which chunks are relevant to the current task, and calls `recall_knowledge(chunk)` to fetch the full content. The tool call and its result appear in the conversation transcript — auditable and included in `/snapshot` outputs.

**Growth:** `/learn` writes new chunk files to `knowledge/`. New chunks are visible to Claude from the next session onward (the registry is built at session init).

**Seeding:** Drop `.txt` files into `knowledge/` with the correct format and they are picked up automatically on next session start. No rebuild step required.
...
**Knowledge files:** `knowledge/` — one `.txt` file per chunk; scanned at session start to build the registry.
